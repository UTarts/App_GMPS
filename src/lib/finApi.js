...
export const feeApi = {
  ...
  getFeeHeads:          (token)           => finApi('get_fee_heads', {}, token, 'GET'),
  getPendingSubmissions:(token)           => finApi('get_pending_submissions', {}, token, 'GET'),
  getAllSubmissions:     (token, status)   => finApi('get_all_submissions', { status }, token),
  ...
  getDefaulters:        (token, m, y)     => finApi('get_defaulters', { month: m, year: y }, token),
  getReports:           (token, type, date) => finApi('get_reports', { type, ...(date ? { date } : {}) }, token),
